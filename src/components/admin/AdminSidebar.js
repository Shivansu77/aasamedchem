...
const navItems = [
  ...
  },
  {
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.941 3.199.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0A5.971 5.971 0 0 0 6 18.719m0 0a9.094 9.094 0 0 1-3.741-.478 3 3 0 0 1 4.682-2.72M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z" />
      </svg>
    ),
  },
  {
      ...
      <div className="px-4 py-4 border-b border-slate-200 flex items-center sm:px-6 md:py-5">
        <Link href="/" className="flex items-center gap-3">
          ...
          <span className="text-lg font-bold tracking-tight text-slate-800">
            AASA <span className="font-light text-slate-500">Admin</span>
          </span>
        </Link>
      </div>
      ...
      <nav className="flex gap-2 overflow-x-auto px-4 py-3 md:flex-1 md:flex-col md:gap-0 md:space-y-1 md:overflow-visible md:py-6">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`transition-colors ${isActive ? "text-brand-600" : "text-slate-400 group-hover:text-slate-500"}`}>
                {item.icon}
              </span>
              {item.label}
              {isActive && (
